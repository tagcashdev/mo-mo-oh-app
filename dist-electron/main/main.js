...
const electronIsDev = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  default: isDev
}, Symbol.toStringTag, { value: "Module" }));
const require$$2 = /* @__PURE__ */ getAugmentedNamespace(electronIsDev);
var lib$2 = { exports: {} };
...
function requireDatabase$1() {
  if (hasRequiredDatabase$1) return database$1;
...
function requireLib$1() {
  ...
  hasRequiredLib$1 = 1;
  lib$2.exports = requireDatabase$1();
...
var dataImporter;
var hasRequiredDataImporter;
function requireDataImporter() {
  if (hasRequiredDataImporter) return dataImporter;
  hasRequiredDataImporter = 1;
  const fetch2 = require$$0;
  const { getDb } = requireDatabase();
  const path = require$$1$1;
  ...
  async function downloadAndSaveImage(imageUrl, cardName, imageApiId, subfolder = "artworks_main") {
    ...
    const relativeImagePath = path.join("card_images", subfolder, localImageName);
    try {
  ...
  async function importAllCardData(mainWindow) {
    ...
    try {
      ...
      const apiResponse = await fetch2(YGOPRODECK_API_URL);
      if (!apiResponse.ok) {
      ...
      for (const cardData of cardsFromApi) {
        ...
        const cardToInsert = {
          ...
          passcode: String(cardData.id).slice(0, 8),
          ...
          first_release_tcg: ((_b = (_a = cardData.misc_info) == null ? void 0 : _a[0]) == null ? void 0 : _b.tcg_date) || null,
          first_release_ocg: ((_d = (_c = cardData.misc_info) == null ? void 0 : _c[0]) == null ? void 0 : _d.ocg_date) || null,
          is_token: ((_e = cardData.type) == null ? void 0 : _e.toLowerCase().includes("token")) ? 1 : 0,
          is_skill_card: ((_f = cardData.type) == null ? void 0 : _f.toLowerCase().includes("skill card")) ? 1 : 0
        };
        let newCardDbId;
        try {
            ...
            for (const setPrint of cardData.card_sets) {
              ...
              if (setId && newCardDbId) {
                insertCardPrintingStmt.run({
                  card_id: newCardDbId,
                  set_id: setId,
                  card_number_in_set: setPrint.set_code,
                  // L'API YGOProDeck utilise set_code pour le numéro dans le set.
                  rarity: setPrint.set_rarity,
                  language: "EN",
                  // L'API principale est en anglais.
                  current_price_nm_eur: parseFloat(setPrint.set_price) || null
                  // L'API fournit set_price
                });
              }
            }
          }
          if (cardData.card_images && cardData.card_images.length > 1) {
            for (let i = 0; i < cardData.card_images.length; i++) {
              ...
              if (artworkLocalPath && newCardDbId) {
                insertAlternateArtworkStmt.run({
                  card_id: newCardDbId,
                  artwork_name: `Artwork API ID ${imgData.id}` + (i === 0 ? " (Principal)" : ""),
                  // Nom plus descriptif
  ...
  dataImporter = { importAllCardData };
  return dataImporter;
}
var hasRequiredMain;
function requireMain() {
  ...
  const path = require$$1$1;
  const isDev2 = require$$2;
  const { initDatabase, getDb } = requireDatabase();
  const { importAllCardData } = requireDataImporter();
  let mainWindow;
  ...
  ipcMain.handle("import-all-cards", async () => {
    ...
    try {
      await importAllCardData(mainWindow);
  ...
  ipcMain.handle("get-all-cards", async () => {
    const db = getDb();
    try {
      const stmt = db.prepare("SELECT * FROM Cards LIMIT 50");
  ...
  ipcMain.handle("create-card-printing", async (event, printingData) => {
    ...
    try {
      ...
      if (existingSet) {
        ...
        const info2 = db.prepare("INSERT INTO Sets (set_name, set_code) VALUES (?, ?)").run(set_name, set_code);
        setId = info2.lastInsertRowid;
      }
      const stmt = db.prepare(`
  ...
  ipcMain.handle("save-printing-collection-details", async (event, { printingId, items }) => {
    ...
    const saveTransaction = db.transaction(() => {
      ...
      const itemIdsFromFrontend = /* @__PURE__ */ new Set();
      for (const item of items) {
        if (item._to_delete && item.id > 0) {
          db.prepare("DELETE FROM UserCollectionItems WHERE id = ?").run(item.id);
          continue;
        }
        if (item._to_delete) continue;
        if (item.id > 0) {
  ...
  ipcMain.handle("update-printing-collection-quantity", async (event, { printingId, status, change }) => {
    if (printingId === void 0 || !status || !["Owned", "Wanted", "Trade"].includes(status) || ![-1, 1].includes(change)) {
